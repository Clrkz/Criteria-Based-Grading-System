<?php 
    ...
    $id = $_SESSION['id'];  
    $x = "select * from userdata where username='$id'";
	$y = mysql_query($x);
    $z = mysql_fetch_array($y);
    $xyz  = $z[1]; 
    $xyza  = $z[2]; 
	$location_uri = $_SERVER['REQUEST_URI'];
	$current_uri = strpos($location_uri,"changepass.php");
	if($xyz==$xyza){  
		if($current_uri == ''){
		header('location: changepass.php');
		}
	} 
	echo "<script>console.log('$location_uri')</script>"; 
	echo "<script>console.log('$current_uri')</script>"; 
